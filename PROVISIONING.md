# How to provision a new environment for blog
...
* Enter domains to secure (for example): `cloudsdk.org`
* Enter webroot path: `/home/deployer/apps/blog/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/cloudsdk.org/fullchain.pem /etc/ssl/blog.crt
ln -s /etc/letsencrypt/live/cloudsdk.org/privkey.pem /etc/ssl/blog.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/blog/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
17 4,16 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/blog/shared/public/
```
